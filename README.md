# Railway Object Storage Download Benchmark

Benchmarks Railway Object Storage / Tigris download behavior by generating synthetic files, uploading each file, signing a GET URL, and asking a remote Worker to download the object concurrently.

Workers act as isolated remote download clients. The Worker consumes the full response body through the runtime Cache API as a short-lived native sink, then returns timing data. The local script measures Worker request wall time, aggregates stats, writes one self-contained HTML report, deletes uploaded objects in `finally` blocks, and removes local temporary fixtures at the end of the run.
...
`.env.local` is ignored by git. It needs Railway credentials and the deployed Worker URL:
...
By default, the benchmark runs Railway Object Storage for every configured file size, concurrency level, and round, then writes one HTML report. It generates temporary `1 MB`, `25 MB`, `50 MB`, and `100 MB` synthetic files. Each file is tested at concurrency `1,10,25,50,75,100`.
...
The HTML summary includes one section per file size. Each section has a per-file MB/s chart and a Railway summary table:

| Concurrency | Errors | P50 | P95 | P99 | Max | MB/s |
|---:|---:|---:|---:|---:|---:|---:|
